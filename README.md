Проект плана проведения семинаров (21.01.2026)
...
Вот детальный план проектирования и создания базы данных для семинара **21.01.2026**. Этот материал является основой для реализации регистрации и авторизации в курсовом проекте (Интернет-магазин).

Формат: **SEMINAR_GUIDE (Инструкция к действию)**
Роль: **METHODIST_WEB**
...
Перед тем как писать SQL, студент должен понимать, какие данные мы храним.
...
*Если база уже была создана на занятии 14.01, этот шаг пропускается или проверяется.*

1.  Зайти в панель управления Beget.
2.  Раздел **MySQL**.
3.  Создать базу данных.
    *   **Имя:** `логин_shop` (например, `ivlev_shop`).
...
Студентам нужно создать первого пользователя (Администратора) вручную, чтобы протестировать форму входа (`login.php`), которую они будут писать на второй паре.
...
**⚡ Лайфхак для студентов (Как получить реальный хеш прямо сейчас):**
...
Студент готов к написанию PHP-кода, если:
...
Если студент делает сложный проект, предложите добавить таблицу `user_profiles` для хранения адреса доставки и привязать её к `users` через `user_id` (связь 1:1), чтобы не перегружать основную таблицу авторизации.
...
План занятия:
    ...
    Самостоятельная работа / Курсовой проект (15 мин):
        ...
        Студенты адаптируют код под свои темы курсовых (например, добавляют поле "Имя" или "Телефон" при регистрации).
...
В рамках подготовки Курсового проекта (Интернет-магазин) студенты должны к следующей неделе:
...
Методические заметки для преподавателя (Ивлев А.Л.):

    Акцент на PHP 8: Напоминать студентам, что мы используем современные стандарты.

    Безопасность: Строго пресекать попытки хранить пароли в чистом виде. Это автоматический "неуд" за этап безопасности.
